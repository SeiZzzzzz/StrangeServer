...
namespace StrangeServerCSharp
{
    public struct TYPacket
    {
        ...
        public TYPacket(byte[] input)
        {
            ...
            int headerLength = et.Length + t.Length + loc.Length * 2;
        ...
        public TYPacket(uint eventTime, string eventType, uint x, uint y, string data) : this(eventTime, eventType, x, y, Encoding.UTF8.GetBytes(data)) { }
        public byte[] Compile
        {
            get
            {
                ...
                byte[] a = Encoding.UTF8.GetBytes(eventType);
                byte[] b = BitConverter.GetBytes(eventTime);
                byte[] c = BitConverter.GetBytes(x);
                byte[] d = BitConverter.GetBytes(y);
        ...
        public uint Length => (uint)(eventTime.ToString().Length + eventType.Length + x.ToString().Length + y.ToString().Length + data.Length);
    }
}
